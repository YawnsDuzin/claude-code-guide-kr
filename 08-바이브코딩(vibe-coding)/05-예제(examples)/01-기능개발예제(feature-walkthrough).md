...
ERD가 바뀌므로 [스키마변경흐름](../../04-워크플로우(workflows)/10-스키마변경흐름(schema-flow).md)을 먼저 실행.

### 에이전트에 투입한 프롬프트

[04-스키마마이그레이션 템플릿](../02-프롬프트템플릿(prompts)/04-스키마마이그레이션(schema).md)을 채움:
...
이제 기능 개발. [01-기능개발 프롬프트](../02-프롬프트템플릿(prompts)/01-기능개발(feature).md) 사용.
...
별도 세션 (컨텍스트 오염 방지). [05-UI구현 프롬프트](../02-프롬프트템플릿(prompts)/05-UI구현(ui).md).
...
- 3단계에서 `toggle is idempotent` 테스트가 실패 → [02-버그수정 프롬프트](../02-프롬프트템플릿(prompts)/02-버그수정(bugfix).md)로 2단계 프롬프트 사용. "원인 먼저" 요청.
