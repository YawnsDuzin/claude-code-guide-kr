...
> 커뮤니티가 만든 서브에이전트 / 스킬 / 커맨드 컬렉션은 수십 개에 이른다. 이 문서는 **무엇을 / 왜 / 어떻게 설치할지** 를 결정하기 위한 실전 참조 가이드다.
...
|---|------|--------|------|----------|---------|-----------------|
| 1 | **wshobson/agents** | subagent + skill + command | 182 agents / 149 skills / 96 commands / 16 orchestrator | 클론 → 수동 복사 | MIT | 프로덕션 규모, 멀티 에이전트 오케스트레이션 필요 |
| 2 | **VoltAgent/awesome-claude-code-subagents** | subagent | 100+ (카테고리 9~10개) | 설치 스크립트 / 수동 복사 | (오픈) | 언어/인프라/보안 등 도메인 전문 에이전트가 필요한 풀스택 |
| 3 | **0xfurai/claude-code-subagents** | subagent | 100+ (16 카테고리) | 클론 → `~/.claude/agents/` | MIT | 일관된 포맷, 비용 의식적 (모델 자동 매핑) |
| 4 | **davepoon/buildwithclaude** | agent + command + hook + skill | 117 agents / 175 commands / 28 hooks / 26 skills | 클론 → 수동 복사 (웹 UI로 검색) | MIT | 한 곳에서 다양한 자산을 검색해서 선택 |
...
| 7 | **anthropics/skills** (공식) | skill | 문서/코드/디자인 스킬 | 클론 → `~/.claude/skills/` 복사 | Apache 2.0 (일부 source-available) | 공식 품질을 신뢰, 문서 조작 등 표준 작업 |
...
- **규모**: 182 에이전트 / 149 스킬 / 96 커맨드 / 16 오케스트레이터
...
- **설치**:
  ...
  cp /tmp/wshobson/agents/<원하는-에이전트>.md ~/.claude/agents/
  ```
  에이전트별로 쪼개져 있어 **필요한 것만** 선별 복사 가능.
- **라이선스**: MIT

**장점**
1. **규모 · 커버리지**: 거의 모든 스택/도메인 대응.
2. **멀티 에이전트 오케스트레이터 16개** — "전체 기능 개발", "보안 강화", "병렬 코드 리뷰 팀" 등 이미 설계된 워크플로우.
3. **에이전트 세분화** — 토큰 절약, 필요한 것만 로드.
...
- **카테고리**:
  ...
  - Research & Analysis — 3
- **설치** (3가지):
  1. 스크립트: `./install-agents.sh` (클론 후, 카테고리 선택 가능)
  2. curl 단발 설치
  3. 수동 복사 → `~/.claude/agents/`
...
- 토큰 비용에 민감한 환경
- 사내 보안 정책이 보수적인 환경 (외부 스크립트 실행 불가 등)
...
- **규모**: 117 agents / 175 commands / 28 hooks / 26 skills
- **카테고리**: 11 (agents), 22 (commands), 8 (hooks) 등
- **설치**:
  ```bash
  git clone https://github.com/davepoon/buildwithclaude.git /tmp/davepoon
  mkdir -p ~/.claude/agents ~/.claude/commands
  cp /tmp/davepoon/agents/<원하는-에이전트>.md ~/.claude/agents/
  cp /tmp/davepoon/commands/<원하는-커맨드>.md ~/.claude/commands/
  ```
  웹 UI(buildwithclaude.com)에서 검색 후 필요한 파일만 선별 복사.
- **라이선스**: MIT
...
2. "개인 사용"을 명시 — 프로덕션 규모 팀 배포에는 다른 선택이 나음.
...
- **설치**: 레포 클론 후 `~/.claude/skills/`로 복사. 각 스킬은 `SKILL.md` + (옵션) 스크립트/리소스 폴더.
- **라이선스**:
...
| **레거시 대형 리팩토링** | wshobson `refactorer` + `reviewer` 오케스트레이터 | 0xfurai `testing-*` | 4-role 협업([02-역할기반협업](../03-주요기능(features)/05-서브에이전트(subagents).md)) 필수 |
| **사내 보안 엄격** | 0xfurai | lst97 | 수동 복사 · 외부 스크립트 미의존 |
...
[04-워크플로우](../04-워크플로우(workflows)/) 의 8단계 공통 루프를 따라, 각 단계에서 "어떤 에이전트가 좋은가"를 정리.
...
- **큰 작업 (6파일+)**: 4-role — Planner → Coder → Reviewer → Tester ([02-역할기반협업](../03-주요기능(features)/05-서브에이전트(subagents).md)). wshobson의 오케스트레이터를 그대로 쓰거나 직접 구성.
...
### 4-1. 전역 수동 복사 (`~/.claude/agents/`) — 권장

레포를 클론해 **홈 디렉토리**에 복사. 모든 프로젝트에서 사용 가능.
...
- anthropics/skills (pdf, docx만 선택) — `~/.claude/skills/`로 복사
...
**운영 방식**: [02-역할기반협업](../03-주요기능(features)/05-서브에이전트(subagents).md)의 Planner/Coder/Reviewer 3-role을 lst97 에이전트로 매핑.
...
| 라이선스 확인 안 함 | 사내 감사 시 문제 | 설치 체크리스트(§4-3) 필수 |
...
- [서브에이전트 §멀티에이전트 패턴](../03-주요기능(features)/05-서브에이전트(subagents).md#멀티에이전트-패턴) — Orchestrator-Worker / Pipeline / Parallel / Debate
- [서브에이전트 §역할 기반 협업](../03-주요기능(features)/05-서브에이전트(subagents).md#역할-기반-협업-4-role-모델) — Planner/Coder/Reviewer/Tester 4-role
- [크로스 도구 핸드오프](../06-비교(comparison)/05-크로스도구핸드오프(cross-tool-handoff).md) — 도구 전환 시 컨텍스트 유지
- [서브에이전트](../03-주요기능(features)/05-서브에이전트(subagents).md) — Claude Code 서브에이전트 기능 자체 설명
- [07-최적화/성능 및 비용 최적화](../07-최적화(optimization)/03-성능최적화(performance).md) — 에이전트 다수 사용 시 토큰 비용
