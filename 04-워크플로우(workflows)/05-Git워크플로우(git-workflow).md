...
> Git 기본 개념과 명령어는 [06-Git기본사용법](./06-Git기본사용법(git-basics).md)을 참조하세요.
...
동료 리뷰어가 같은 팀이면 암묵 가정(implicit assumption)을 공유합니다. 그래서 **"당연히 X일 것"** 이라는 가정을 둘 다 검증하지 않고 넘어갑니다. 에이전트는 맥락을 강제로 문서에서만 얻기 때문에, 암묵 가정을 드러내주는 경우가 많습니다.
