...
| 좋은 프롬프트 구조 | [`02-프롬프트/`](../02-프롬프트(prompts)/) | 범용 템플릿 + 작업 유형별 7종 |
| 적절한 작업 분할 | [`01-워크플로우/`](.) (본 폴더) | 작업 유형별 단계 체크리스트 |
...
| 유형 | 언제 | 플레이북 |
|------|------|---------|
| 기능 개발 | 새 기능을 추가할 때 | [02-기능개발흐름](./02-기능개발흐름(feature-flow).md) |
| 버그 수정 | 기존 동작이 깨졌을 때 | [03-버그수정흐름](./03-버그수정흐름(bugfix-flow).md) |
| 리팩토링 | 동작은 맞지만 구조가 나쁠 때 | [04-리팩토링흐름](./04-리팩토링흐름(refactoring-flow).md) |
| 스키마 변경 | DB 구조를 바꿔야 할 때 | [05-스키마변경흐름](./05-스키마변경흐름(schema-flow).md) |
| UI 구현 | 화면 / 레이아웃 작업 | [06-UI구현흐름](./06-UI구현흐름(ui-flow).md) |
...
3. **프롬프트 템플릿 로드** — `02-프롬프트/`에서 유형에 맞는 템플릿을 복사.
...
- **보조 에이전트**: GPT Codex, Cursor, Windsurf, Aider 등과 병용 가능 (→ [04-에이전트/03-크로스에이전트핸드오프](../04-에이전트(agents)/03-크로스에이전트핸드오프(cross-agent-handoff).md))
- **문서 포맷**: Markdown + Mermaid (→ [05-포맷](../05-포맷(formats)/README.md))
